...
void process_WREV(char* cmd_str, A_UINT16 cmd_len)
{
    ...
    A_UINT32 wlan_ver = 0x1102;
    AT_PTF("Hostless UART \nFirmware version :  %d.%d.%d.%d.%d\n",
...
void process_WPM(char* cmd_str, A_UINT16 cmd_len)
{
    ...
    do 
    {
        if((*cmd_str == '?') || (cmd_len < 1))
        {
            ...
            if(mode ==REC_POWER)
                AT_PTF("Power mode   =   REC_POWER\n");
            else
                AT_PTF("Power mode   =   MAX_PERF_POWER\n");
            break;
        }
...
void process_WWEPKEY(char* cmd_str, A_UINT16 cmd_len)
{
    ...
    do {
        ...
        A_SSCANF(cmd_str, "%d,%s", &input, token_buf);
...
void process_WMI(char *pCommand, unsigned short commandSize)
{
  ...
  commandInfo  = ((WMI_CMD_HDR *)pCommand)->commandInfo;
