...
AJ_Status AJ_DisconnectWiFi(void)
{
    ...
    A_STATUS disconnect_status = A_OK;
    ...
    if (connectState != AJ_WIFI_DISCONNECTING) {
        ...
        disconnect_status = qcom_disconnect(qcom_DeviceId);

        if (disconnect_status != A_OK) {
...
AJ_Status AJ_PrintFWVersion()
{
...
#ifdef USE_MAC_FOR_GUID
    {
        ...
        if (A_OK == qcom_mac_get(qcom_DeviceId, macAddr)) {
            ...
            AJ_InfoPrintf(("WiFi MAC address   :  0x%s\n", buffer));
        }
    }
#endif
}
...
void AJ_CreateNewGUIDFromMAC(uint8_t* rand, uint32_t len)
{
    ...
    if (A_OK == qcom_mac_get(qcom_DeviceId, macAddr)) {
        ...
        AJ_WarnPrintf(("AJ_CreateNewGUIDFromMAC(): Unable to get MAC address (%s), falling back to AJ_RandBytes\n", AJ_StatusText(status)));
