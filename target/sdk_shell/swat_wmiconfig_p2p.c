...
CMD_P2P_EVENT_INFO *app_p2p_getNextP2PEventInfo(void)
{
    ...
    if(NULL != p2pEventNode.pEventHead)
    {
        ...
        tx_mutex_get(&(p2pEventNode.eventQueueMutex), TX_WAIT_FOREVER);
        ...
        /* Release the mutex */
        tx_mutex_put(&(p2pEventNode.eventQueueMutex));
    }
    /* Return pointer to the event or NULL*/
    return(pTemp);
}
/* Add event info to the tail of the p2p event queue */
A_STATUS app_p2p_queueP2PEnventInfo(A_UINT8 device_id, P2P_EVENT_ID event_id, A_UINT8 *pBuffer, A_UINT32 Length)
{
     ...
     */
    if((pNewP2pEventInfo = ((CMD_P2P_EVENT_INFO *)mem_alloc(sizeof(CMD_P2P_EVENT_INFO) + Length))) == NULL){
...
A_STATUS app_p2p_event_go_neg(A_UINT8 device_id, A_UINT8 * pEvtBuffer, int len)
{
    ...
    if (0 == p_tmp->status)
    {
        ...
        if (p_tmp->role_go == 1)
        {
            ...
            /* Wait for first connect event. It means AP is up and running. Then
             * start WPS 
             */
            qcom_sta_connect_event_wait(device_id, &state);
            qcom_wps_start(device_id, 1, ((p_tmp->wps_method == WPS_PBC)? WPS_PBC:0), szpin);
        }
        else {
...
/* APP callback function registered with CDR. This function handled P2P events in Proxy thread context.
 * This callback should not do any blocking calls.
*/
int app_p2p_event_handler_cb(A_UINT8 device_id, P2P_EVENT_ID event_id, A_UINT8 *pBuffer, A_UINT32 Length, void *arg)
{
     ...
     */
     if(A_ERROR == (status = app_p2p_queueP2PEnventInfo(device_id, event_id, pBuffer, Length)))
     {
        printf(" Out of Memory: Dropping the P2P event\n");
        return 0;
     }
     /* event to APP thread: P2P event */
     tx_event_flags_set(&event_flags_0, APP_P2P_EVENT_FLAG, TX_OR);
     return 1;
}

/* P2P Event handler function */
int app_p2p_event_handler(A_UINT8 device_id, P2P_EVENT_ID event_id, A_UINT8 *pBuffer, A_UINT32 Length)
{
    switch (event_id) {
...
/* p2p event handler that is executed in the APP thread context.
 * Events are push to p2p event queue in Proxy thread context and processed in APP context.
 * Since some of the P2P events are blocking.
 */
A_STATUS app_p2p_process_event(void)
{
   CMD_P2P_EVENT_INFO *pP2pEventInfo = NULL;
   /* fetch the next P2P event from the P2P event queue */
   if(NULL == (pP2pEventInfo = app_p2p_getNextP2PEventInfo()))
      return A_OK;

   if(!pP2pEventInfo->Length){
      mem_free(pP2pEventInfo);
      return A_ERROR;
   }
   /* P2P event handler */
   app_p2p_event_handler(pP2pEventInfo->device_id, pP2pEventInfo->event_id, ((A_UINT8 *)&(pP2pEventInfo->pBuffer[0])), pP2pEventInfo->Length);
   /* Free the event info memory. */
   mem_free(pP2pEventInfo);
   return A_OK;
}

/* app_p2p_delete_event_queu */
void app_p2p_delete_event_queue(void)
{
   CMD_P2P_EVENT_INFO *pP2pEventInfo = NULL;
   /* fetch the next P2P event from the P2P event queue */
   while(NULL != (pP2pEventInfo = app_p2p_getNextP2PEventInfo()))
	   mem_free(pP2pEventInfo);
...
int cmd_wlan_p2p(int argc, char *argv[])
{
    ...
    if(!strcmp(argv[1], "on")) {
        ...
        tx_mutex_create(&(p2pEventNode.eventQueueMutex), "p2p_event_q_mutex", TX_NO_INHERIT);
        qcom_p2p_func_register_event_handler(app_p2p_event_handler_cb, NULL);
        cmd_p2p_set_wps_pin("12345670");
    }
    else if(!strcmp(argv[1], "off")) {
        qcom_p2p_func_register_event_handler(NULL, NULL);
        tx_mutex_delete(&(p2pEventNode.eventQueueMutex));
	    app_p2p_delete_event_queue();
