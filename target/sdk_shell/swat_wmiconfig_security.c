    ...
    switch (uEventID)
    {
        case QCOM_WPS_PROFILE_EVENT:
        {
            ...
            if (QCOM_WPS_STATUS_SUCCESS == p_tmp->error_code) {
                ...
                /*
                  * Before doing wps-connection it is better to add some delay to avoid interlocking of off-chop locker.
                  * qcom_wps_set_credentials will get an off-chop locker for channel setting.
                  * qcom_wps_connect will try to get another off-chop locker for scanning.
                  */
...
swat_wmiconfig_ap_wps_set(A_UINT8 device_id)
{
     if(gDeviceContextPtr[device_id]->hiddenFlag == 1)
     {
        SWAT_PTF("In AP hidden mode, WPS should be disabled.\n");
        return;
     }
...
{
    qcom_wps_config_state_set(currentDeviceId, mode);
    return;
}
#endif
...
swat_wmiconfig_wep_passowrd_set(A_UINT8 device_id, A_CHAR * passwd)
{
    ...
    status = qcom_sec_set_passphrase(device_id, passwd);
	if (status != A_OK)
	{
	   SWAT_PTF("wrong password, the length should be between 8 and 64\n");
	}
}
