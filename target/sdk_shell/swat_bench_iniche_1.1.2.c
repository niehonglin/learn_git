...
swat_tcp_rx_handle(STREAM_CXT_t * pStreamCxt)
{
    ...
    while (1) {
        ...
        if(v6_enabled){
            ...
            if((clientFd = swat_accept(pStreamCxt->socketLocal, (struct sockaddr *) &clientAddr, &len)) < 0){
            /* Close Socket */
            SWAT_PTF("Failed to accept socket %d.\n", clientFd);
            goto QUIT;
           }
...
void
swat_bench_tx_test(A_UINT32 ipAddress, IP6_ADDR_T ip6Address, A_UINT32 port, A_UINT32 protocol, A_UINT32 pktSize,
