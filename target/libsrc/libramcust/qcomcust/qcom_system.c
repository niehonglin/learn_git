 ...
 */
#if defined(AR6002_REV72) || defined(AR6002_REV74)
...
void _qcom_sys_ap_client_num_set(void);

void _qcom_sys_tunables_apply(void)
{

    /*Apply NUM_SOFTAP_CLIENTS Tunable param to adjust the allowed number of softap clients */
    {
        _qcom_sys_ap_client_num_set();
    }

    /*Apply QCOM_MEMPOOL_CHUNKSIZE Tunable param to adjust the qcom_mem_pool chunk size*/
    {
        extern unsigned int qcom_mem_pool_default_sz;
        unsigned int chunksize_k=QCOM_TUNE_VAL_OR_DFL(SYS_TUNE_QCOM_MEMPOOL_CHUNKSIZE,2);
        qcom_mem_pool_default_sz=chunksize_k*1024;
    }
...
_qcom_pre_init(void)
{
...
#if defined(CONFIG_STRRCL_FEATURE)
    STRRCL_INIT(gdevp);
#endif
#endif
    ...
    if (argos_feature_enabled == TRUE) {
         ...
         * station. So if P2P is enabled, we initiate WPS and P2P moduler here first.
         */
#if defined(P2P_ENABLED)
        if(HOST_INTEREST->hi_option_flag3 & HI_OPTION_USE_OFFLOAD_P2P) {
            extern WPS_CONTEXT *pg_wps;
            extern A_STATUS _qcom_p2p_init(int dev_id);

            qcom_wps_enable(0, 1);

            /*
             * When starting WPS, the public-key has to be calculated and that will spend a long
             * time period(about 5 seconds). So the public-key is pre-calculated during WPS_INT
             * to reduce duration time of WPS procedure.
             * Normally the public-key calculating will be triggered by CDR-READY event.
             * But if WPS is initiated in application after system bootup. WPS moduler can not
             * catch CDR-READY event. In this case that is done directly at the end of WPS_INIT.
             * If WPS is initiated here, it can catch the CDR-READY event.
             * So here we disarm the timer for public-key calculating.
             */
            A_UNTIMEOUT(&pg_wps->key_timer);
    ...
    extern void _qcom_init_ipstack(void);
    _qcom_init_ipstack();
...
_qcom_pre_patch(void)
{
    ...
    /* Enable the following to add the SharkSSL ECC patch */
#ifdef ENABLE_SHARKSSL_ECC_SUPPORT
    extern void register_SharkSSL_ECC_patch(void * install_patch_cb);
    extern void SharkSsl_patch_install(void);
    register_SharkSSL_ECC_patch((void *)SharkSsl_patch_install);
#endif
...
_qcom_sys_dbg_uart_set(A_UINT32 uiinput_param)
{
    ...
    HOST_INTEREST->hi_dbg_uart_txpin = 24;
    HOST_INTEREST->hi_dbg_uart_rxpin = 25;
    A_ENABLE_DEBUG_PRINT();

#else  /* else of defined (FPGA) */

    HOST_INTEREST->hi_desired_baud_rate = HI_OPTION_BAUDRATE;
    /* DO NOT USE PIN7 for UART as that pin is conflicting with FLASH Quad mode
     * Access resulting in corruption of data read from flash */
    HOST_INTEREST->hi_dbg_uart_txpin = HI_UART_NORMAL_TXPIN; //HI_UART_DBG_TXPIN;
...
#define MAX_AP_CLIENT_NUM  10
#else
#define MAX_AP_CLIENT_NUM  4
#endif

    A_UINT32    num_clients = 0;

    num_clients = QCOM_TUNE_VAL_OR_DFL(SYS_TUNE_NUM_SOFTAP_CLIENTS, MAX_AP_CLIENT_NUM);

    HOST_INTEREST->hi_option_flag2 &= ~(HI_OPTION_AP_CLIENT_CNT_MASK << HI_OPTION_AP_CLIENT_CNT_SHIFT);

    HOST_INTEREST->hi_option_flag2 |= (num_clients << HI_OPTION_AP_CLIENT_CNT_SHIFT);
...
#if 0
    /* configure to hostless mode */
    {
        A_UINT32 reg;
        /*
         * SDIO_PINS_EN reset value is "1". USB uses selected pins for GPIO
         */
        reg = A_GPIO_REG_READ(SDIO_ADDRESS);
        reg &= (~SDIO_PINS_EN_MASK);
        A_GPIO_REG_WRITE(SDIO_ADDRESS, reg);
    }
#endif
}
...
extern install_patch_t   install_patch_cb[MAX_NUM_FEATURE_INSTALL_PATCHES];

void qcom_sys_targinit(){
...
/*Temporarily enable the section of code below to avoid breaking Alljoyn apps.
 * Application should call qcom_sys_targinit from app_start before intializing any globals*/
#if 1
    install_patch_t   patch_cb[MAX_NUM_FEATURE_INSTALL_PATCHES];

    /* HACK!!!  HACK!!! HACK!!! HACK!!! HACK!!!
     * Backup the data in install_patch_cb[] in stack and restore it
     * after A_INIT(). A_INIT() initialized all globals to 0. The data
     * in install_patch_cb[] gets lost because of that. This is just
     * a workaround which needs to be addressed in a better way */
    A_MEMCPY(patch_cb, install_patch_cb, sizeof(patch_cb));

    /* A_INIT must be called from app_start.c. Calling it here will
     * cause all globals that were initialized before this to become 0 again.
     * Move call this into all the applications for R3.3 */
    if (!A_INIT()) {
        A_PRINTF("A_INIT() FAILED.\n");
        return;
    }
    /* Restore install_patch_cb from stack */
        A_MEMCPY(install_patch_cb, patch_cb, sizeof(patch_cb));
...
#if defined(AR6002_REV74) && defined(P2P_ENABLED)
#ifdef CONFIG_P2P_FEATURE
    ...
    patch_domain_install();
     ...
     /********************************************
      * revert CL 1413105 (System time (time_ms()) doesn't restart from 0 after watchdog reset)
      * because it will cause the problem as below:
      * 1:DUT can't connect with ref-ap on singleband
      * 2:DUT hung when enter CLI "iwconfig" on singleband
      **********************************************/
#if 0
     extern A_UINT64 cmnos_milliseconds_era;
     extern unsigned int cmnos_prev_lowfreq_timer;

	{
		A_old_intr_t old_intr;
		unsigned int lowfreq_timer;

#if defined(CONFIG_STALE_LF_TIMER_WAR)
		{
			extern void (* plf_timer_stabilize)(void);
			plf_timer_stabilize();
		}
#endif
		A_INTR_DISABLE(&old_intr);
        A_RTC_SOC_REG_WRITE(LF_TIMER_CONTROL0_ADDRESS, 0x1);
		A_DELAY_USECS(500);
		lowfreq_timer = A_RTC_SOC_REG_READ(LF_TIMER_COUNT0_ADDRESS);
		cmnos_milliseconds_era = 0;
		cmnos_prev_lowfreq_timer = lowfreq_timer;
...
int qcom_watchdog(int enable, int timeout)
{
    ...
    switch(enable){
        ...
        case APP_WDT_ENABLE:
            ...
            cmnos_timer_wdt_timeout_secs = WDT_EXPIRE_INTERVAL;
			wdt_usr_flag = 0;
            break;
...
{
    if (enable){
    A_POWER_SYSSLEEP_CONTROL(1); /* Permit sleep */
    }
    else {
    A_POWER_SYSSLEEP_CONTROL(0); /* disallow sleep */
    }
    return;
}
...
{
    extern devh_t* devArray[];
    wlan_set_bmisstime(devArray[device_id],numBeacons,bmissTime);
    return  A_OK;
}

A_STATUS qcom_enable_ani(A_UINT32 enable)
{
     whalEnableAni(enable);
     return A_OK;
}

A_STATUS qcom_active_partition(A_UINT32 partition_index)
{
    A_INT32 ret;
    if (partition_index < 1 || partition_index >= PARTITION_MAX) {
        return A_EINVAL;
    }
    ret = ota_partition_active(partition_index);
    return ret;
}
