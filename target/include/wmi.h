...
typedef enum {
   ...
   WMI_SET_DFS_ENABLE_CMDID,   
    ...
    WMI_PARAM_GET_CMDID,                /* F0F2*/
    WMI_PARAM_SET_CMDID,                /* F0F3*/

    WMI_WPS_SET_CONFIG_STATE_CMDID,                /* F0F4*/
    WMI_WLAN_SET_PROBEREQ_ENABLE_CMDID , /*FOF5*/
    WMI_GET_COUNTRY_CODE_CMDID  ,        /*F0F6*/
    WMI_WLAN_WPS_INIT_KEY_CMDID              /*F0F7 */
...
#define WMI_CCX_SFA_IE             3 /* ieType to set CCX SFA IE */
#define WMI_CCX_DIAG_REASON_IE     4 /* ieType to set CCX Diagnostic Reason IE */

#define WMI_CCX_ID_VER_IE               0x03
#define WMI_CCX_ID_SFA_IE               0x14
#define WMI_CCX_ID_DIAG_REASON_IE       0x12
#define WMI_CCX_ID_MHDR_IE              0x10
#define WMI_CCX_DIAG_REASON_MAX         0xFF
#define CCX_SFA_MFP_MASK        (0x1)           /* CCXv5 support S67 for MFP*/
#define CCX_SFA_DC_MASK         (0x1 << 1)      /* CCXv5 support S64 for Diagnostic Channel */
#define CCX_SFA_LS_MASK         (0x1 << 2)      /* CCXv5 support S69 for Location services */
#define CCX_SFA_EBR_MASK         (0x1 << 3)      /* CCXv5 support S70 for Expedited Bandwidth Requests */
...
typedef enum {
    ...
    WMI_PARAM_GET_REPLY_EVENTID,            /*0x903F*/ 
...
/*
 CCXv5 	S65 - Client reporting
*/
typedef  PREPACK struct {
    A_UINT8 	group_id;  /*0:Client Profile, 1:Operating Parameters, 2 :Manufacturer's Information, 3:Client Capability*/    
    A_UINT8 	status;     /* 0 means success , the same as A_OK
                                         1 means continue, host should send another query command,
                                         other values are errors */   
    A_UINT16    length;    /*Length of reporting informaion*/
    A_UINT8     clientInfor[1]; /*Buffer to carry client reporting informaion*/
}POSTPACK WMI_QUERY_PHY_INFO_EVENT;

/*
 CCXv5 	S66 - Roaming and Real-time Diagnostics	Roaming Log Element Data
*/
typedef PREPACK  struct {
    A_UINT8 		src_bssid[6];     		/*Source BSSID	*/
    A_UINT8 		target_bssid[6];     	/*Target BSSID	*/    
    A_UINT16    	trans_time;							/*Transition Time	*/
    A_UINT8 		trans_reason;         					/*Transition Reason	*/
    A_UINT16    	trans_result;							/*Transition Result	*/
}POSTPACK WMI_CCX_REALTIME_ROAM_EVENT;
/*
 * developer commands
 */
...
/*
 * Commands structure to support CCXv4 .
 */ 

typedef PREPACK struct {
    A_UINT8 enable; // 1 for enable & 0 for disable
} POSTPACK WMI_CCX_FRAME_REPORT_CMD;

typedef enum {
    WMI_CCX_FEATURE_CCKM_ROAM_UPLOAD = 0x01,
    WMI_CCX_FEATURE_BEACON_REPORT_UPLOAD = 0x02,
    WMI_CCX_FEATURE_FRAME_REPORT_UPLOAD = 0x04,
} WMI_CCX_HOST_FEATURE_CONFIG;

#define DEFAULT_CCX_HOST_FEAURE_CONFIG_FLAGS         (WMI_CCX_FEATURE_CCKM_ROAM_UPLOAD)

typedef PREPACK struct {
       A_UINT16 feature_config;
} POSTPACK WMI_CCX_HOST_FEATURE_CONFIG_CMD;
